...
/** GPX filename prefix; the ISO date keeps repeated downloads easy to identify. */
const GPX_FILENAME_PREFIX = 'itineraire-swiss-trail-planner';
...
export function createRouteGpx(
  ...
  const trackPoints = coordinates
    ...
    .join('\n');
  const routeName = escapeXml('Itinéraire Swiss Trail Planner');
 ...
 */
export function downloadRouteGpx(steps: RouteStep[]): void {
  const generatedAt = new Date();
  const gpxDocument = createRouteGpx(steps, generatedAt);
